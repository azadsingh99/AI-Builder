...
const modelCreation = (models) => {
    try {
        for (let model of models) {
            const name = model.name;
            const fields = model.fields

            console.log('First Model Creation Started......')
            const responseOfModelGeneratopr = generateModelIndividually(name, fields);
            console.log(responseOfModelGeneratopr)
        }
    } catch (err) {
        console.log(err);
        return err;
